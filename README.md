...
Baril, T., Feurtey, A., Croll, D. 2026. TobyBaril/EarlGreyParTEA: Earl Grey ParTEA v0.2.2. *Zenodo*. https://doi.org/10.5281/zenodo.18935538
...
- 🔍 **Version-Agnostic EarlGrey Detection** — ParTEA auto-detects the EarlGrey `scripts/` directory from any conda installation (any version ≥7.3.0). No manual path configuration needed.
...
- 🖥️ **LSF Cluster Submission** *(new in v0.1.9, experimental)* — Pass `--lsf --lsf-queue <queue>` to dispatch each rule as an individual `bsub` job via `snakemake-executor-plugin-lsf`. Per-job CPUs, memory, and wall-time are read from the same rule resource declarations used for SLURM, so no rule changes are needed. Concurrency is controlled by `--lsf-jobs`. **Note:** `snakemake-executor-plugin-lsf` is a community plugin not maintained by the Snakemake core team; this feature is therefore experimental.
...
ParTEA now requires **EarlGrey ≥7.3.0**, which ships RepeatMasker 4.2.4, RMBlast 2.17.1,
RepeatModeler 2.0.9, and the standalone **FamDB 3.0.0** package (Dfam 4.0).

#### What changed

In EarlGrey 7.3.0, FamDB is no longer embedded inside RepeatMasker's `share/` tree.
It is now installed as a separate conda package at `share/famdb-3.0.0/`, with Dfam 4.0
HDF5 files at `share/famdb-3.0.0/Libraries/famdb/`. RepeatMasker is pre-configured by
the conda post-install hook — no `perl ./configure` step is required.

| Step | Old (≤7.2, Dfam 3.9) | New (7.3.0, Dfam 4.0) |
|------|----------------------|----------------------|
| Install | `mamba install earlgrey-partea` | `mamba install earlgrey-partea` |
| Download libraries | manual `curl` + `perl ./configure` | `download_dfam.py` (interactive) |
| Manual symlinks | Required (`famdb/`, `RMRB.embl`) | Not required |
| Marker file | `.earlgrey.config.complete` | Not needed |

#### Files changed

- **`scripts/on_start_functions.py`** — startup validation auto-detects the standalone
  `famdb-*/Libraries/famdb/` directory; falls back to the old embedded path for backward
  compatibility with earlgrey ≤7.2 environments.
- **`rules/lib_construct.smk`** — `extract_repeatmasker_library` updated to detect the
  new FamDB path automatically.
- **`scripts/configure_dfam.sh`** — rewritten to reflect the `download_dfam.py` setup flow.
- **`conda/meta.yaml`** — minimum EarlGrey version raised to `≥7.3.0`.

---

## Previous Release (v0.1.9)

### LSF Cluster Submission (Experimental)

ParTEA can now submit each Snakemake rule as an individual LSF (`bsub`) job using the community-maintained `snakemake-executor-plugin-lsf` package. Enable by passing `--lsf --lsf-queue <queue>` to any of the three wrapper scripts.

Key behaviours:
- `--executor lsf` is activated automatically when `--lsf` is passed
- Each rule's existing `threads`, `mem_mb`, and `runtime` resource declarations are reused without modification — no rule changes are required
- By default the plugin converts `mem_mb` to a **per-core** memory request (`mem_mb / threads`); set `SNAKEMAKE_LSF_MEMFMT=perjob` to request the full `mem_mb` as a per-job total instead
- `--latency-wait 60` is applied automatically to handle NFS filesystem latency
- One automatic retry at 2× memory on failure
- `--slurm` and `--lsf` cannot be used together

New flags (all three entry points):

| Flag | Description |
|------|-------------|
| `--lsf` | Enable LSF submission mode `[EXPERIMENTAL]` |
| `--lsf-queue QUEUE` | LSF queue (required; or set `lsf_queue:` in config) |
| `--lsf-jobs N` | Max concurrent LSF jobs (default: genomes × 3, capped at 200) |
| `--lsf-project PROJ` | LSF project string (optional) |
| `--lsf-extra "FLAGS"` | Extra raw `bsub` flags |

New optional config keys (only used when `--lsf` is passed):

```yaml
lsf_queue: ""         # queue to submit to (required when using --lsf)
lsf_project: ""       # project string (leave empty if not required)
lsf_extra: ""         # any extra bsub flags, e.g. "-R 'select[type==X86_64]'"
```

`snakemake-executor-plugin-lsf` is added as a run dependency in the conda package alongside `snakemake-executor-plugin-slurm`.

> ⚠️ This feature is marked experimental because `snakemake-executor-plugin-lsf` is a community plugin not maintained by the Snakemake core team, and has not been tested by the ParTEA developers (we do not have access to an LSF cluster). See [LSF Cluster Submission (Experimental)](#️-lsf-cluster-submission-experimental) for full usage details.
...
- [Changes in Latest Release](#-changes-in-latest-release-v020)
...
- [LSF Cluster Submission (Experimental)](#%EF%B8%8F-lsf-cluster-submission-experimental)
...
**✨ Magic Feature:** ParTEA automatically detects your EarlGrey installation (any version ≥7.3.0) and adapts on the fly. Update EarlGrey anytime - no config changes needed!
...
**ParTEA requires EarlGrey ≥7.3.0 (Dfam 4.0 / FamDB 3.0.0).** The pipeline checks for a
configured Dfam library on startup and will fail with clear instructions if it is missing.

After installing via conda/mamba, download the Dfam 4.0 library partitions using the
interactive tool provided by the `famdb` package:

```bash
# Activate your environment
mamba activate partea  # or your environment name

# Run the interactive download tool
download_dfam.py
```

`download_dfam.py` will guide you through selecting which Dfam 4.0 partitions to download
(curated consensus sequences, HMMs, uncurated sequences, etc.) and will write the HDF5
files to the correct location automatically. No `perl ./configure` or manual symlinks are
needed — RepeatMasker is pre-configured by the conda package.
...
```bash
# Confirm that Dfam HDF5 files are present after download_dfam.py completes:
ls $CONDA_PREFIX/share/famdb-*/Libraries/famdb/
# Should show: dfam40.0.h5  dfam40.curated.consensus.0.h5  (and others if downloaded)
```
...
## 🖥️ LSF Cluster Submission (Experimental)

> ⚠️ **Experimental feature.** LSF support uses the community-maintained [`snakemake-executor-plugin-lsf`](https://snakemake.github.io/snakemake-plugin-catalog/plugins/executor/lsf.html), which is not part of the official Snakemake organisation. This feature has not been tested by the ParTEA developers (we do not have access to an LSF cluster). Please report any issues at https://github.com/TobyBaril/EarlGreyParTEA/issues.

ParTEA can submit each rule as an individual LSF job using `snakemake-executor-plugin-lsf`.

### How it works

- `--executor lsf` is activated automatically when `--lsf` is passed
- Each rule declares its own `threads`, `mem_mb`, and `runtime` (in minutes); these map directly to LSF's `-n`, `-R rusage[mem=...]`, and `-W` in the `bsub` call
- `--latency-wait 60` is applied automatically to handle NFS filesystem latency
- On failure, a rule is automatically retried once at 2× memory (`mem_mb = base * attempt`)
...
| `--lsf-jobs N` | Max concurrent LSF jobs (default: genomes × 3, max 200) |
| `--lsf-project PROJ` | LSF project string (optional) |
| `--lsf-extra "FLAGS"` | Extra raw `bsub` flags |

### Thread semantics in LSF mode

In LSF mode, `-t` sets **CPUs per job** — each genome job gets its own allocation. Concurrency is governed by `--lsf-jobs` instead. This is identical to the behaviour in SLURM mode.

### Memory per core vs per job

By default, `snakemake-executor-plugin-lsf` converts the rule's `mem_mb` value to a **per-core** memory request (i.e. `-R rusage[mem=<mem_mb/threads>]`), which is what most LSF clusters expect. If your cluster instead accepts **per-job** memory totals, set the environment variable before running:

```bash
export SNAKEMAKE_LSF_MEMFMT=perjob
earlGreyParTEA -c my_config.yaml -t 16 --lsf --lsf-queue myqueue
```

### Config options

LSF settings can be placed in the config file (so you don't need to pass them on the command line every time):

```yaml
# LSF cluster settings (only used with --lsf flag) [EXPERIMENTAL]
lsf_queue: "normal"       # queue to submit to
lsf_project: "myproject"  # project string (leave empty if not required)
lsf_extra: ""             # any extra bsub flags
```

### Example

```bash
# Pass queue on the command line
earlGreyParTEA -c my_config.yaml -t 16 --lsf --lsf-queue normal

# Or set lsf_queue in config.yaml and just use --lsf
earlGreyParTEA -c my_config.yaml -t 16 --lsf

# With project string and concurrent job limit
earlGreyParTEA -c my_config.yaml -t 16 --lsf --lsf-queue normal --lsf-project myproject --lsf-jobs 50
```

### Mutual exclusion

`--slurm` and `--lsf` cannot be used together. Using both will print an error and exit.
...
- `snakemake-executor-plugin-lsf` (required for `--lsf` mode; included in the conda package) *(experimental)*
- Python ≥3.9,<3.13
